...
namespace PssHighLowTemperature.ViewModel
{
    public class MainViewModel : ViewModelBase
    {
        ...
        public bool IsStandard
        {
            ...
            set { _isStandard = value; RaisePropertyChanged(() => IsStandard); }
        }
        ...
        public MainViewModel()
        {
            ...
            XElement xeCoeOfRx = XDoc.Root.Element("CoeOfRx");
            Coefficient = Convert.ToDouble(xEle.Value.Trim());
            CalOfDOA = Convert.ToDouble(xEleOpm.Value.Trim());
            CoeOfRx = Convert.ToDouble(xeCoeOfRx.Value.Trim());
            //获取Com列表
                ...
                Thread thread = new Thread(() =>
                  {
                      ...
                      {
                          MessageBox.Show(string.Format("初始化失败,请检查设备连接：{0}", ex.Message), "系统提示");
                          IsTestEnable = false;
                          IsIniReady = false;
                      }
            ...
            try
            {
                ...
                Thread thread = new Thread(() =>
                {
                    try
                    {
                        ...
                        if (IsStandard)
                        {
                            MessageBoxResult result = MessageBox.Show("正在进行基准键测量，将不存储数据", "系统提示", MessageBoxButton.OKCancel);
                        ...
                        if (Math.Abs(Ag8610Temp) > 0.5)
                        {
                            MessageBox.Show(string.Format("模组温度偏差大于0.5，当前温度:{0}。请校验后重试",Ag8610Temp), "系统提示");
                            IsTestEnable = true;
                        ...
                        if (IsStandard == false)
                        {
                            ...
                            Thread.Sleep(200);
                            int time = 0;
                            while (serbuf[0] == 0 && serbuf[1] == 0 && time < 2)
                            {
                                Thread.Sleep(1000);
                                State = PssDOA.ReadDDM(PssBase.CARDID_3, ENDSIGN, 0xA1, 68, count, serbuf);
                                Thread.Sleep(200);
                                ...
                                if (time == 2)
                                {
                                    MessageBox.Show("读取Sn失败，请重试", "系统提示");
                                    IsTestEnable = true;
                            ...
                            if (long.TryParse(sn.Trim(), out long SNint))
                            {
                                MessageBox.Show("读取sn失败,请重试", "系统提示", MessageBoxButton.OKCancel);
                        ...
                        {
                            MessageBox.Show("功率不在范围内，请重试!", "系统提示");
                        ...
                        if (!IsStandard)
                        {
                            ...
                            if (_rxPoint1 == 0)
                            {
                                MessageBox.Show("读取Rx1失败，请重试！", "系统提示");
                            ...
                            if (_rxPoint2 == 0)
                            {
                                MessageBox.Show("读取Rx2失败，请重试！", "系统提示");
                            ...
                            if (_rxPoint3 == 0)
                            {
                                MessageBox.Show("读取Rx3失败，请重试！", "系统提示");
                            ...
                            switch (TempLevel)
                            {
                                case "-40":
                                    if (Temp > 0)
                                    {
                                        MessageBox.Show("温度测试条件选择错误", "系统提示");
                                        return;
                                    }
                                    if (Temp < -15)
                                    {
                        ...
                        if (sen == 0 || sen == 1)
                        {
                            MessageBox.Show("读取Sen失败，设备初始化可能未成功。请重新初始化设备", "系统提示");
                            return;
                        }
                        //Sen指数形式显示
                        TestParas.Sensitivity = sen.ToString("E");
                        //Sen非指数显示
                        Sen = Math.Log10(sen).ToString("f2");
                        RaisePropertyChanged(() => TestParas);
                        ...
                        TestParas.IsSensitivity == true == true)
                            {
                    ...
                    {
                        //捕抓测试异常
                        MessageBox.Show(string.Format("测试失败：{0}", ex.Message), "系统提示");
                        TestParas.SN = string.Empty;
                        IsTestEnable = true;
                        TestParas.IsTempPass = false;
                        RaisePropertyChanged(() => TestParas);
        ...
        private void ExecuteDoaCalibration()
        {
            ...
            if (Math.Abs(_coeOfRx) > 5)
            {
                MessageBox.Show("请检查光功率计是否连接,然后重新校验", "系统提示");
                return;
            }
            ...
            XDoc.Root.Element("CoeOfRx").SetValue(CoeOfRx);
            XDoc.Save("Configure.xml");
        }
        /// <summary>
        /// 系统校验命令
        ...
        public void SaveData()
        {
            ...
            TestParas.ProductType = ProductType;
            bool isEmpty = false;
